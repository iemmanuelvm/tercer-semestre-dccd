...
try:
    ...
    raise ImportError(
        "GeomLoss no está instalado. Instala con: pip install geomloss"
    ) from e
...
def _collect_numpy_safe_globals():
    ...
    try:
        ...
        pass
    # NumPy 2.x: clases paramétricas en numpy.dtypes (p.ej. Float64DType, Int32DType, etc.)
    try:
...
# Intentamos registrar globalmente (si la API existe), y además usaremos el context manager al cargar
try:
...
def _safe_torch_load(path, map_location="cpu", trust: bool = False):
    """
    1) torch.load con weights_only=True (por defecto en PyTorch 2.6)
    2) Reintento con allowlist ampliada usando safe_globals(...)
...
class ResUNetTCN(nn.Module):
    ...
    def _encode_features(self, x):
        # Encoder + bottleneck + atención -> features [B,C]
        skips = []
        h = self.stem(x)
    ...
    def encode(self, x):
        # Para adaptación: solo features (sin decodificar)
...
# =========================
#   Entrenamiento + adaptación (GeomLoss)
# =========================
    ...
    for epoch in range(1, epochs + 1):
        ...
        for xb, yb in train_loader:
            ...
            if domain_adapt:
                ...
                # GeomLoss admite tensores 2D [N,D] y [M,D]
